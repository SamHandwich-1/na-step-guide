...
The user specifically chose "Integrated" mode because they WANT these frameworks. Make sure every response draws from at least one framework in addition to NA principles.`;
  ...
  // Experience-based opening guidance
  let openingGuidance = '';
  
  if (userProfile.experienceLevel === 'first') {
    openingGuidance = `## Opening This Session (FIRST TIME working steps)
${userProfile.name} is new to formal step work. Your opening should:
- Be warm and welcoming - this is vulnerable territory
- Quote the step text directly from NA literature
- Keep it simple and grounded in the basics of addiction/recovery
- Ask one focused, accessible question

Example opening for Step ${currentStep}:
"Hey ${userProfile.name}, welcome to Step ${currentStep}. The NA Basic Text tells us: '${stepData.text}' 

How do you feel when you hear that? What does '${stepData.themes[0]}' mean to you in your own experience?"

Keep questions tight and focused on their direct experience with addiction. Don't overwhelm with concepts yet.`;
  } else if (userProfile.experienceLevel === 'some') {
    openingGuidance = `## Opening This Session (SOME step work experience)
${userProfile.name} has started step work before but may not have finished. Your opening should:
- Acknowledge they're returning to familiar territory
- Quote NA literature but also begin connecting to deeper themes
- Be curious about what's different this time
- Reference spiritual principles for this step: ${stepData.principles.join(', ')}

Example opening for Step ${currentStep}:
"${userProfile.name}, you've been here before - Step ${currentStep}: '${stepData.text}'

What brings you back to this step now? Sometimes we circle back because we're ready to go deeper. The principle of ${stepData.principles[0]} often reveals itself differently each time we return."`;
  } else {
    openingGuidance = `## Opening This Session (CYCLING through steps again)
${userProfile.name} is working the steps again in ongoing recovery. Your opening can:
- Meet them at a deeper level - they know the basics
- Draw from multiple sources: NA literature, Patrick Carnes' "A Gentle Path Through the Twelve Steps", Stoic philosophy, Buddhist wisdom
- Focus on the spiritual principles: ${stepData.principles.join(', ')}
- Explore how this step lands differently with more recovery time

Example opening for Step ${currentStep}:
"${userProfile.name}, coming back to Step ${currentStep} with time in recovery is a different experience. The Basic Text says '${stepData.text}' - but Patrick Carnes invites us to see this step through the lens of ${stepData.themes[0]} as a doorway, not just a concept to understand.

What's alive for you in this step right now? Where does ${stepData.principles[0]} want your attention?"

You can reference Stoic ideas about what we control, Buddhist concepts of attachment and suffering, or IFS parts work as appropriate.`;
  }
